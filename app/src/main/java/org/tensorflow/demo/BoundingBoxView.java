...
public class BoundingBoxView extends View {
    ...
    private Bitmap TensorFlowBitmap = null;
    ...
    public void onDraw(final Canvas canvas) {
        ...
        //1/11/2018 Chih-Yuan correct it
        float view_height = view_height_temp;  //752, for Zenbo
        float view_width = view_width_temp;    //1002, for Zenbo
        ...
        float INPUT_SIZE = 448; //(float) TensorFlowImageListener.getInputSize();  //448, for Zenbo

        float size_multiplier_y = view_width / INPUT_SIZE;
        float size_multiplier_x = size_multiplier_y;
        float offset_y = -(INPUT_SIZE * size_multiplier_x - view_height) / 2;
        float offset_x = 0;

        //2/23/2018 Chih-Yuan: I want to draw the bitmap processed by tensorflow.
        if( TensorFlowBitmap != null) {
            canvas.drawBitmap(TensorFlowBitmap, 0, 0, null);
        }
        ...
        if (results != null) {
            for (final Recognition recog : results) {
                ...
                //1/12/2018 Chih-Yuan, there should be a flip horizontally
                bounding_x = INPUT_SIZE - bounding_x;
